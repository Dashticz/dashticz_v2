	...
	if(coin['source']=='litebit'){
		$.getJSON('https://cors-anywhere.herokuapp.com/https://www.litebit.eu/system/live-updates',function(data){
			...
			if(typeof(coin['amount'])!=='undefined' && parseFloat(coin['amount'])>0){
				var html='<div class="col-xs-6 col-data">'+coin['key'].toUpperCase()+'<br /><strong class="title">'+symbol+number_format(data[varname],2,sepdecimal,septhousands)+'</strong><br>';
				html+='<span class="state">'+coin['amount']+'</span>';
				html+='</div>';
				html+='<div class="col-xs-6 col-data col-currency">'+symbol+number_format(data[coin['key']+'RateSellRound']*coin['amount'],2,sepdecimal,septhousands)+'</div>';
			}
			else {
		...
		$.getJSON('https://api.coinmarketcap.com/v1/ticker/'+coin['key']+'/?convert='+coin['currency'],function(data){
			...
			if(typeof(coin['amount'])!=='undefined' && parseFloat(coin['amount'])>0){
				var html='<div class="col-xs-6 col-data">'+data[0]['symbol']+'<br /><strong class="title">'+symbol+number_format(data[0][varname],2,sepdecimal,septhousands)+'</strong><br>';
				html+='<span class="state">'+coin['amount']+'</span>';
				html+='</div>';
				html+='<div class="col-xs-6 col-data col-currency">'+symbol+number_format(data[0][varname]*coin['amount'],2,sepdecimal,septhousands)+'</div>';
			}
			else {
